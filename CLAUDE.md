...
**`.claude/WORKLOG.md` 를 읽으세요.** 이 파일(구조 지식)과 달리 워크로그(진행 상황·미해결 이슈)는
자동으로 로드되지 않습니다. 읽지 않으면 지난 작업을 모르는 상태로 시작하게 됩니다.

- 사용자가 "이어서 하자 / 어디까지 했지 / 지난 작업 확인" 이라고 하면 → 워크로그를 읽고 브리핑.
- `/panur-resume` 스킬에 복원·기록 절차가 더 자세히 있지만, **스킬 없이도 위 한 줄만 지키면 됩니다.**
- 작업을 마칠 때는 워크로그를 갱신하세요(형식은 `/panur-resume` 참고).
